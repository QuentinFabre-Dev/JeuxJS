...
export const EVENTS = [
  ...
  'verdict', // { ref, verdict }   the critic ruled on a streamed finding
...
/**
 * Splits a stream buffer into complete frames.
 *
 * A network chunk can end anywhere, including halfway through a frame, so the
 * trailing fragment is handed back to be prepended to the next chunk. Losing
 * that fragment is how a finding silently disappears from a review.
 */
export const splitFrames = (buffer) => {
  const frames = [];
