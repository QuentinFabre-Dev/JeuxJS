...
/**
 * What a finished review actually cost, from the token counts the API
 * reported.
 *
 * The estimate shown before the run is only worth something if the real figure
 * follows it. Cached input is billed at a tenth, which is the whole point of
 * keeping the system prompt byte-identical across calls.
 */
export const actualCost = (usageByTier = {}) => {
  let dollars = 0;
  let inputTokens = 0;
  let outputTokens = 0;
  let cachedInputTokens = 0;

  for (const [tier, usage] of Object.entries(usageByTier)) {
    const model = modelFor(tier);
    const fresh = Math.max(0, (usage.inputTokens ?? 0) - (usage.cachedInputTokens ?? 0));
    dollars +=
      (fresh * model.inputPerMillion) / 1e6 +
      ((usage.cachedInputTokens ?? 0) * model.inputPerMillion) / 1e7 +
      ((usage.outputTokens ?? 0) * model.outputPerMillion) / 1e6;
  ...
  return { dollars, inputTokens, outputTokens, cachedInputTokens };
};
