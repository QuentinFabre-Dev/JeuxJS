...
export const checkTerminology = (sentences, { glossary = [] } = {}) => {
  ...
  for (const [acronym, use] of firstUse) {
    ...
    candidates.push({
      ...
      skill: 'consistency',
      suggestion: use.text,
      // Phrased to avoid an elision: "défini seulement" takes both
      // "en page 4" and "plus loin dans le document" without a French glitch.
      explanation: `« ${acronym} » est employé ici, mais défini seulement ${elsewhere(use, definition, 'plus loin')}. Définissez-le à sa première occurrence.`,
      priority: 'low',
      confidence: 0.9,
    });
  }
