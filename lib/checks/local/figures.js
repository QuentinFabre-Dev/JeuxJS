...
/**
 * "en page 4" — or, when both sit on the same page, a direction.
 *
 * A page number is only useful when it points somewhere else; on a one-page
 * document "défini qu'en page 1" while standing on page 1 reads as a bug.
 */
const elsewhere = (from, to, direction) =>
  from.page === to.page ? `${direction} dans le document` : `en page ${to.page}`;
    ...
    for (const match of sentence.text.matchAll(AMOUNT)) {
      ...
      if (!seen) {
        ...
        candidates.push({
          ...
          explanation: `Le même montant est écrit « ${seen.spelling} » ${elsewhere(sentence, seen.sentence, 'plus haut')} et « ${spelling} » ici. Gardez une seule écriture.`,
          priority: 'low',
          confidence: 0.85,
        });
      }
      ...
      if (label.length >= 6) {
        ...
        if (!previous) {
          ...
          candidates.push({
            ...
            explanation: `« ${label} » vaut ${previous.spelling} ${elsewhere(sentence, previous.sentence, 'plus haut')} et ${spelling} ici. Vérifiez lequel fait foi.`,
            priority: 'high',
            confidence: 0.8,
          });
        }
  ...
  if (dateFormats.size > 1) {
    ...
    for (const other of others) {
      candidates.push({
        ...
        explanation: `Le document mélange les formats de date : ${first.format.label} ${elsewhere(other.sentence, first.sentence, 'plus haut')}, ${other.format.label} ici.`,
        priority: 'low',
        confidence: 0.9,
      });
    }
